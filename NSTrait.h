...
/* This category add traits support to Objective-C. Traits allows methods to 
 * to another class.
 * [..] a trait is a collection of methods, used as a "simple conceptual model for 
 * structuring object-oriented programs" similar to mixins.
 * Traits provide a simple way to create classes that reuse behavior 
 * from software components.
 * An object defined as a trait is created as the composition of methods available 
 * in several other objects, which allows for code reuse from various classes
 * without requiring multiple inheritance. In case of a naming collision, 
 * when the composed objects have several methods with the same name, 
 * the programmer must explicitly disambiguate which one of those methods will 
 * be used in the trait; thus manually solving the "diamond problem" of repeated 
 * inheritance. This is different from other composition methods in 
 * object-oriented programming, where conflicting names are
 * automatically resolved by scoping rules. [...]
 ...
 *
 * This is just to supress the warnings. Won't 
 *
 * @dyanamic name;
 *
 ...
 * @end
 *
 * ..And voilà. Now is possible to class [- sayHello] on an object of class 
 * ExampleTarget.
 */
